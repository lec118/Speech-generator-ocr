...
export function buildPrompt({
  ...
  const sections: string[] = [
    ...
    "  - 예: \"먼저, 난임 보장이에요.\", \"다음은 수술 보장입니다.\", \"마지막으로 암 보장입니다.\"",
    ...
    "  - 예: \"먼저, 난임 보장이에요.\", \"다음은 수술 보장입니다.\", \"포함돼요.\", \"보장됩니다.\"",
    ...
    "### 필수 출력 포맷",
    "1. **제목만** 굵게 표기하세요.",
    "   - 첫 줄: **무배당 퍼스트케어 암보험 – 핵심 보장 내용**",
    "   - `제목:` 같은 레이블이나 `#`, `-` 기호를 붙이지 마세요.",
    "",
    "2. **본문은 순수 텍스트만 사용하세요.**",
    "   - 빈 줄 하나 후, 세 번째 줄부터 본문 시작",
    "   - ❌ 절대 금지: `**굵게**`, `*이탤릭*`, `-` 불릿, `1.` 번호, `#` 제목, ` ``` ` 코드",
    "   - ✅ 허용: 순수 텍스트, 마침표(.), 쉼표(,), 느낌표(!), 물음표(?), 줄바꿈",
    "",
    "3. **본문 내 강조는 절대 금지**",
    "   - \"**난임 보장**이에요\" (X) → \"난임 보장이에요\" (O)",
    "   - \"**수술 보장**입니다\" (X) → \"수술 보장입니다\" (O)",
    "   - \"**암 보장**입니다\" (X) → \"암 보장입니다\" (O)",
    "",
    "4. 최종 출력 형식: **제목(굵게)** + 빈 줄 + 본문(순수 텍스트)",
    "   - SSML 태그(`<break time=\"1s\">`)도 절대 사용 금지",
    "",
    "**정확한 출력 예시:**",
    "```",
    ...
    "",
    "먼저, 난임 보장이에요. 저렴한 보험료로 남성 난임을 보장하고, 10년간 함께 보장하지요. 다음은 수술 보장입니다. 탈장이나 요로결석 같은 질병 수술에 대비할 수 있고, 특정 비뇨기계 수술도 포함돼요. 마지막으로 암 보장입니다. 중입자 치료비 1억 원, 그리고 전립선 검사비까지 새로 추가됐어요.",
    "",
    "30세 남성 기준, 월 보험료는 약 3만 1천 원대로 확인되지요. 이 상품으로 한 번에 대비할 수 있습니다.",
    "```",
    "",
    "**주의: 본문에 별표(**) 절대 사용 금지!**",
    "- 위 예시처럼 본문은 순수 텍스트만 사용",
    "- 제목 외에는 어떤 마크다운도 사용하지 마세요",
    "",
    "---",
    "",
    "## ▶️ 생성 지시문",
